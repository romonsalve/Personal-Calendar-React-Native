import * as Actions from '../actions/bookings_actions';

export function bookingsById(bookings) {
...
export function removeBooking(id, bookings) {
...
export function bookingsState(state = { isFetching: false, bookings: {} }, action) {
  switch (action.type) {
...
export function bookingsByStatus(state = {}, action) {
  const {status} = action;
  switch (action.type) {
    case Actions.REQUEST_BOOKINGS:
    case Actions.RECEIVE_BOOKINGS:
...
export function selectedBooking(state = {}, action) {
  switch (action.type) {
